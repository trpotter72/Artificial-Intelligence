...
(defparameter *common-binding-check* t "When true, no two variables can bind to the same object")
...
(defun unify (x y &optional (bindings no-bindings) bind-onto-numbers ignore-not-eq bind-constant-to-var)
  "See if x and y match with given bindings."
  (cond ((eq bindings fail) fail)
        ((eql x y) bindings)
	((and ignore-not-eq (not (variable-p x)) (not (variable-p y)) (not (consp x)) (not (consp y)) (not (numberp x)) (not (numberp y)))
	 (when (or log-exp-equal*)
	   (log-message (list "Attempting to bind constants, ~A, ~A in Unify.~%" x y)))
	 (cond ((and (not (get-binding x bindings)) (not (get-binding y bindings)))
		(when (or log-exp-equal*)
		  (log-message (list "SUCCESS.~%")))
		(extend-bindings x y bindings))
	       ((or (and (not (null (get-binding x bindings)))
			 (not (eq y (norvig-lookup x bindings))))
		    (and (not (null (get-binding y bindings)))
			 (not (eq x (norvig-lookup y bindings)))))
		(when (or log-exp-equal*)
		  (log-message (list "~A: Failure because ~A not bound to ~A.~%" (get-binding x bindings) x y))
		  (log-message (list "~A: Failure because ~A not bound to ~A.~%" (get-binding y bindings) y x)))
		fail)
	       (t
		bindings)))
	((and (variable-p y) bind-constant-to-var)
	 (unify-variable x y bindings bind-onto-numbers bind-constant-to-var))
	((variable-p x) (unify-variable x y bindings bind-onto-numbers bind-constant-to-var))
        ((variable-p y) (unify-variable y x bindings bind-onto-numbers bind-constant-to-var))
        ((and (consp x) (consp y))
         (unify (rest x) (rest y) 
                (unify (first x) (first y) bindings bind-onto-numbers ignore-not-eq bind-constant-to-var)
		bind-onto-numbers ignore-not-eq bind-constant-to-var))
        (t fail)))

(defun unify-variable (var x bindings &optional bind-onto-numbers bind-constant-to-var)
  "Unify var with x, using (and maybe extending) bindings."
  (cond ((get-binding var bindings)
         (unify (norvig-lookup var bindings) x bindings bind-onto-numbers bind-constant-to-var))
        ((and (variable-p x) (get-binding x bindings))
         (unify var (norvig-lookup x bindings) bindings bind-onto-numbers bind-constant-tovar))
        ((and *occurs-check* (occurs-check var x bindings))
         fail)

	;; fail if any other variable is already bound to x
	((and *common-binding-check* (rassoc x bindings))
	 fail)
